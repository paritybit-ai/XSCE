...
void ServerComputeHashOutputs(u64 width, u64 sender_size, uint64_t bucket2, 
        ...
        const std::string& ip, uint32_t port, std::vector<xsce_ose::block>* oprf_values)
{
    // IOService ios;
    // Endpoint ep(ios, ip, port, EpMode::Server, "Run");
...
extern std::vector<block> PreDealPSIIds(const std::vector<std::string>& ids);
extern std::vector<block> HashIdToBlocks(const uint8_t* hash_ids, int dataByteLen, int id_num);
extern std::vector<block> HashIdToBlocks(const std::vector<util::Buf128>& hash_ids);
int64_t OprfPsiServer::OprfPsiAlg(std::vector<util::Buf128>&& hashBuf, uint64_t rmtNeles, OptAlg* optAlg) {
    uint64_t receiver_size = rmtNeles;
    std::vector<block> sender_set;
    LOG_INFO(optAlg->logger, "begin oprf server");
    if (ids_.size() == 0) { // 外部已经将id hash过了，直接生成sender_set,
        ...
        sender_set = HashIdToBlocks(hashBuf);
    } else {
        // 先hash，在生成sender_set;
        LOG_INFO(optAlg->logger, "oprf server predeal psi ids");
        sender_set = PreDealPSIIds(ids_);
        receiver_size = GetRecevierSize(sender_set.size(), ip_, port_);
    }
    if (offline::USE_OFFLINE_CHANNEL) {
        LOG_INFO(optAlg->logger, "oprf server offline mode");
    } else {
        LOG_INFO(optAlg->logger, "oprf server online mode");
    }

    hashBuf.clear();
    hashBuf.shrink_to_fit();
    return __OprfPsiAlg(std::move(sender_set), receiver_size, optAlg);
}
...
int64_t OprfPsiServer::OprfPsiAlg(uint8_t *hashBuf, uint64_t neles, uint64_t rmtNeles, OptAlg* optAlg)
{
    ...
    if (ids_.size() == 0) { // 外部已经将id hash过了，直接生成sender_set,
        ...
        int dataByteLen = hashLen / (sizeof(uint8_t) * 8);
        sender_set = HashIdToBlocks(hashBuf, dataByteLen, sender_size);
    } else {
        // 先hash，在生成sender_set;
        LOG_INFO(optAlg->logger, "oprf server predeal psi ids");
        sender_set = PreDealPSIIds(ids_);
...
int64_t OprfPsiServer::__OprfPsiAlg(std::vector<block>&& sender_set, uint64_t rmtNeles, OptAlg* optAlg) {
    auto sender_size = sender_set.size();
    LOG_INFO(optAlg->logger, "oprf server ComputeSendSet");
    auto send_set = ComputeSendSet(hash1LengthInBytes, sender_size, commonSeed, commonSeed1, sender_set);
    // 释放不再使用变量的内存;
    sender_set.clear();
    sender_set.shrink_to_fit();
    malloc_trim(0);

    LOG_INFO(optAlg->logger, "oprf server SenderBaseOTs");
    auto sender_base_ot_result = SenderBaseOTs(width, inertalSeed, inertalSeed1, ip_, port_);
    LOG_INFO(optAlg->logger, "oprf server ServerExchangeMetrics");
    ...
    // 释放不再使用变量的内存;
    send_set.clear();
    send_set.shrink_to_fit();
    sender_base_ot_result.ot_messages.clear();
    sender_base_ot_result.ot_messages.shrink_to_fit();
    // sender_base_ot_result.choices.reset();
    {
        sender_base_ot_result.choices.reset();
        auto tmp = sender_base_ot_result.choices;
        std::swap(tmp, sender_base_ot_result.choices);
    }

    LOG_INFO(optAlg->logger, "oprf server ServerComputeHashOutputs");
    std::vector<xsce_ose::block>* oprf_values = nullptr;
    if (save_ov_) {
        oprf_values = &oprf_values_;
    }
    ServerComputeHashOutputs(width, sender_size, bucket2, trans_hash_inputs, hashLengthInBytes, ip_, port_, oprf_values);
    //ch.close();
    //ep.stop();
    //ios.stop();
    return 0;
