...
{
    auto all_hashes = ClientComputeHashOutputs(hash_length_in_bytes, bucket2, width, receiver_size, trans_hash_inputs);
...
std::vector<IdHash> IdsToHash(const std::vector<std::string>& ids) {
    std::vector<IdHash> rlt(ids.size());
...
std::vector<block> PreDealPSIIds(const std::vector<std::string>& ids) {
    ...
    std::vector<block> block_ids(hash_ids.size());
    if (hash_ids.size() == 0) {
        return block_ids;
    }

    // int dataByteLen = hashLen / (sizeof(uint8_t) * 8);
    int dataByteLen = sizeof(hash_ids[0]);
    for (uint64_t i = 0; i < hash_ids.size(); ++i)
    {
        if (8 == dataByteLen)
        {
            block_ids[i] = oc::toBlock(hash_ids[i].buf[0]);
        }
        else if (16 == dataByteLen)
        {
            block_ids[i] = oc::toBlock(hash_ids[i].buf[0], hash_ids[i].buf[1]);
        }
        else
        {
            block_ids[i] = oc::toBlock((i + 1) * 5);
        }
    }
    return block_ids;
}
...
int64_t OprfPsiClient::OprfPsiAlg(uint8_t *hashBuf, uint64_t neles, uint64_t rmtNeles, OptAlg* optAlg)
{
    ...
    if (ids_.size() == 0) {
        ...
        receiver_set.resize(receiver_size);
        int dataByteLen = hashLen / (sizeof(uint8_t) * 8);
        uint64_t *dataBase1;
        uint64_t *dataBase2;
        uint8_t *dataBuf = hashBuf;
        for (uint64_t i = 0; i < receiver_size; ++i)
        {
            if (8 == dataByteLen)
            {
                dataBase1 = (uint64_t *)(dataBuf + i * dataByteLen);
                receiver_set[i] = oc::toBlock(*dataBase1);
            }
            else if (16 == dataByteLen)
            {
                dataBase1 = (uint64_t *)(dataBuf + i * dataByteLen);
                dataBase2 = (uint64_t *)(dataBuf + i * dataByteLen + 8);
                receiver_set[i] = oc::toBlock(*dataBase1, *dataBase2);
            }
            else
            {
                receiver_set[i] = oc::toBlock((i + 1) * 5);
            }
        }
    } else {
    ...
    // PRNG prng(oc::toBlock(inertalSeed,inertalSeed1));
    // block tmp1 = oc::toBlock(inertalSeed,inertalSeed1);
    // std::array<uint8_t, 16> tmp2 = tmp1.as<uint8_t>();
    // block tmp3 = oc::toBlock(tmp2.data());
    // PRNG prng(tmp3);
    ...
    auto pair_result = ReceiveHashOutputsAndComputePSI(width, receiver_size, trans_hash_inputs, bucket2, hashLengthInBytes, sender_size, ip_, port_);
    result_.clear();
